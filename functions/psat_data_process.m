...
function [freq_impz,angle_impz,flow_impz,t_psat] = psat_data_process(Varout,T,dt)
...
for i = 1 : 3
    % frequency
    y_psat(:,i) = Varout.vars(tp_s:tp_e,2*i)-1;
    % rotor angle
    delta0(i) = Varout.vars(2,2*(i-1)+1);
    y_psat(:,i+3) = Varout.vars(tp_s:tp_e,2*i-1)-Varout.vars(2,2*i-1);
end
% frequency
